defmodule Table do
  ...
  def manage_resources(forks, waiting \\ []) do
    ...
    if length(waiting) > 0 do
      ...
      IO.puts(
        "#{length(waiting)} philosopher#{if length(waiting) == 1, do: ~c"", else: ~c"s"} waiting: #{Enum.join(names, ", ")}"
      )
    ...
    receive do
      ...
      {:give_up_seat, free_forks, _} ->
        ...
        IO.puts("#{length(forks)} fork#{if length(forks) == 1, do: ~c"", else: ~c"s"} available")
