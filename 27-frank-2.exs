...
defmodule Frank do
  ...
  defmodule Path do
    defmacro get(path, contents) do
      ...
      quote bind_quoted: [path: path, contents: contents] do
    ...
    defmacro __before_compile__(_env) do
      quote do
        def handle(_, data) do
          IO.puts(~c"here default!")
          response(404, ~c"not defined")
        end
      end
    end

    def redirect(path, code \\ 302) do
      body = [~c"redirecting you to <a href=\"", path, ~c"\">", path, ~c"</a>"]
    ...
    def response(code, body, headers \\ []) do
      ...
      headers =
        [code: code, content_length: Integer.to_charlist(IO.iodata_length(body))] ++ headers
