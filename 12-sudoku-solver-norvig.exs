defmodule SudokuSolver do
  ...
  def grid_values(grid) do
    chars = for c <- grid, c in @cols or c in ~c"0.", do: c
    unless count(chars) == 81, do: raise(~c"error")
  ...
  defp eliminate_vals_from_square(values, square, vals_to_remove, board) do
    ...
    if MapSet.intersection(Enum.into(vals, MapSet.new()), Enum.into(vals_to_remove, MapSet.new()))
       |> any? do
  ...
  def search(values, board) do
    if all?(board.squares, fn s -> count(Map.get(values, s)) == 1 end) do
      ...
      {square, _count} =
        map(board.squares, &{&1, count(Map.get(values, &1))})
...
defmodule SudokuSolverTest do
  ...
  test "solve easy" do
    grid1 = ~c"..3.2.6..9..3.5..1..18.64....81.29..7.......8..67.82....26.95..8..2.3..9..5.1.3.."
    solved = solve(grid1)

    assert solved ==
             ~c"483921657967345821251876493548132976729564138136798245372689514814253769695417382"

    print(grid1, solved)
  end

  test "solve hard" do
    grid2 = ~c"4.....8.5.3..........7......2.....6.....8.4......1.......6.3.7.5..2.....1.4......"
    solved = solve(grid2)

    assert solved ==
             ~c"417369825632158947958724316825437169791586432346912758289643571573291684164875293"
