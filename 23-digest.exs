...
defmodule MiscTest do
  ...
  test "sha1" do
    ...
    assert :sha1.hexstring(~c"foo") == ~c"0BEEC7B5EA3F0FDBC95D0DD47F3C5BC275DA8A33"
  end

  test "md5" do
    # Erlang has crypto built-in for md5, but no way natively to convert to hex
    # so again Mr. Felix to the rescue!
    assert :crypto.hash(:md5, ~c"foo") |> :sha1.bin2hex() == ~c"ACBD18DB4CC2F85CEDEF654FCCC4A4D8"
  end
end
